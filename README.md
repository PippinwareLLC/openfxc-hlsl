...
5. Command-line defines: use `-D NAME` or `-D NAME=VALUE` (repeatable) to seed macros before preprocessing. Macro bodies trim trailing `//` comments so replacements don’t accidentally comment out following code.
6. Diagnostics: reported spans still match the preprocessed stream, but each diagnostic now carries an `origin` (file/span) mapped through `#include` so errors in included files point back to that file; sample sweeps echo diagnostics with origin info to test output. Sample fixtures can provide `includes.json` to add per-tree include search paths (e.g., DXSDK utility headers). Non-HLSL includes (extensions other than `.hlsl`/`.fx`/`.fxh`) are treated as opaque whitespace so C/C++ helper headers do not poison the parse.

Current output includes full lexing for SM1-SM5 and FX constructs; parsing now covers era-specific declarations (samplers/sampler_state, semantics/register/bindings, cbuffer/tbuffer, class/interface/struct bodies, typedefs), FX technique/pass bodies (syntax-only), inline asm blocks, postfix/prefix ++/-- and compound assignments, cast and conditional (`?:`) expressions, FX resource bindings (`Texture[0] = <TextureName>;`), FX9/FX10/FX11 technique forms, and compile invocations (`VertexShader = compile vs_1_1 Foo();`, `vertexfragment Foo = compile_fragment vs_2_0 Bar(true);`) with a `CompilationUnit` AST and diagnostics. Schema matches `docs/TDD.md`. C-style integer modifiers (`unsigned`/`signed`/`long`/`short`/`shared`) are understood in declarations and loop initializers for DXSDK parity.
Variable declarations support multiple declarators per statement (e.g., `float2 dx, dy;`), inline `sampler_state` initializers for all sampler types (sampler, sampler1D/2D/3D/CUBE), and brace initializer expressions inside FX passes (e.g., `MaterialAmbient = {1,1,1,1};`).
Keywords are matched case-insensitively while preserving original spelling; mixed-case `Half` or other capitalized keywords remain identifiers so DXSDK samples (e.g., `Half` vectors, `Sampler` names) stay valid while lowercase keywords still bind.

...
6. Diagnostics: reported spans still match the preprocessed stream, but each diagnostic now carries an `origin` (file/span) mapped through `#include` so errors in included files point back to that file; sample sweeps echo diagnostics with origin info to test output.
